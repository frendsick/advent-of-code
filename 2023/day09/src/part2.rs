pub fn solve(input: &str) -> String {
    input
        .lines()
        .map(parse_numbers_separated_by_spaces_reversed)
...
fn parse_numbers_separated_by_spaces_reversed(input: &str) -> Vec<i64> {
    // Parse numbers from input line from right to left
    input
        .split_whitespace()
